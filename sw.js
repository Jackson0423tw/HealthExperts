...
const C='healthsync-rollback-27R';
...
self.addEventListener('activate',e=>e.waitUntil(caches.keys().then(keys=>Promise.all(keys.filter(k=>k!=='healthsync-rollback-27R').map(k=>caches.delete(k)))).then(()=>self.clients.claim())));
